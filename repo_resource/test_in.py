...
class TestIn(unittest.TestCase):
    def _checkout_project(self, post_sync_hook=False, scp_remote=False):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            payload = b'\x00Test payload\n'
            (project / 'payload.bin').write_bytes(payload)
            git(project, 'add', 'payload.bin')
            if post_sync_hook:
            ...
            git(project, 'commit', '-s', '-m', 'Add project fixture')
    ...
    def test_post_sync_hook_runs(self):
        self._checkout_project(post_sync_hook=True)

    def test_scp_remote_rewrite(self):
        self._checkout_project(scp_remote=True)
